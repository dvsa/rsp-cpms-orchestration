export default (penalties, penaltyType, constants) => {
	return penalties.map((penalty, index) => {
		return {
			...
			amount: penalty.penalty_amount.toFixed(2),
			net_amount: penalty.penalty_amount.toFixed(2),
			tax_amount: Number(0.00).toFixed(2),
			allocated_amount: penalty.penalty_amount.toFixed(2),
			tax_code: 'O',
			tax_rate: '0',
			sales_reference: penalty.vehicle_reg,
			product_reference: 'RoadSidePayments',
			product_description: penaltyType,
			invoice_date: new Date(Date.now()).toISOString().split('T')[0],
			receiver_reference: `${penalty.penalty_reference}_${penaltyType}`,
